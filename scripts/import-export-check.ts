...
console.log("parseTextImport — newline + tab")
{
  ...
  assert(
    r.cards[0].front === "front1" && r.cards[0].back === "back1",
    "card 1 parsed"
  )
  assert(
    r.cards[1].front === "front2" && r.cards[1].back === "back2",
...
console.log("serializeTextExport — collapses multi-line content")
{
  ...
  assert(
    out ===
      "line one line two\tback with several lines\nspaced out\talready fine",
  ...
  assert(
    back.cards.length === 2,
    "collapsed export parses back to same card count"
  )
  assert(
  ...
  assert(
    back.cards[1].front === "spaced out",
    "second card front trimmed+collapsed"
  )
  assert(
...
console.log("serializeTextExport — whitespace-only sides")
{
  ...
  assert(
    out === "front\t",
    "whitespace-only front omitted, NBSP back flattened"
  )
}
...
console.log("parseTextImport — CSV quoting")
{
  ...
  assert(r.cards[1].back === 'a "quoted" back', '"" escape inside quoted back')
}
...
console.log("parseTextImport — multi-line quoted cell")
{
  ...
  assert(r.cards[0].back === "line1\nline2", "first multi-line cell preserved")
...
console.log("parseTextImport — record separator starting with a quote")
{
  ...
  assert(
    r.cards.length === 2 && r.skipped === 0,
    "quote-leading separator still splits records"
  )
  assert(
...
console.log("round-trip — serialize then parse")
{
  ...
  assert(
    r.cards[1].front === "Q2" && r.cards[1].back === "A2 with, comma",
    "comma in back round-trips"
  )
