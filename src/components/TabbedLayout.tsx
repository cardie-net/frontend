...
export function TabbedLayout({ tabs, defaultTabId, title }: TabbedLayoutProps) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col md:min-h-[70vh]">
      {title && (
        <h1 className="text-4xl font-black mb-8 text-foreground uppercase tracking-tight">
          {title}
        </h1>
      )}
      <div className="flex-grow bg-background text-foreground rounded-2xl border-2 border-border-heavy shadow-[8px_8px_0px_var(--color-border-heavy)] overflow-hidden flex flex-col">
        <div
          className={`flex-grow flex flex-row w-[200%] md:w-full transition-transform duration-300 ease-out md:p-2 md:gap-2 ${isMobileViewContent ? '-translate-x-1/2 md:translate-x-0' : 'translate-x-0'}`}
        >
          {/* Left sidebar - Tabs */}
          <div className="w-1/2 md:w-72 md:rounded-2xl bg-foreground/5 p-4 md:p-6 flex flex-col gap-3 shrink-0">
            {tabs.map((tab) => {
          ...
          {/* Right side - Content */}
          <div className="w-1/2 md:w-auto flex-grow flex flex-col bg-foreground text-background md:rounded-2xl">
            {/* Mobile Back Button */}
